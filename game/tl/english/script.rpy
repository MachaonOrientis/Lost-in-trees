# TODO: Translation updated at 2020-10-05 21:30


# game/script.rpy:100
translate english start_e6973fd0:

    # t "Так-так-так. Еще один заплутавший."
    t "Well, well, well. Another lost in the woods."

# game/script.rpy:106
translate english start_b9ec91d0:

    # t "Вижу, ты попал сюда из другого мира. Много странников сюда уже забрело. Не боишься повторить их судьбу?"
    t "I see, you are from some other world! Lots of wanderers have already disappeared here. Aren't you afraid to meet the same fate?"

# game/script.rpy:126
translate english label2_7a147e1f:

    # e "Неужели заблудился? Даже дерево ориентируется лучше, чем ты."
    e "Did you really get lost? Even a tree has a better sense of direction than you."

# game/script.rpy:133
translate english label2_20250f49:

    # e "Все время оказываешься в одном и том же месте? Тебе поможет наш новый продукт: Берестяной навигатор!"
    e "Keeping going in circles? Our brand new product is designed to help you! Buy a navigator made of the birch bark at astounding prices!"

# game/script.rpy:143
translate english label3_6a1f4c7b:

    # d "Ты заблудился? Выглядишь усталым, я бы сказало - изможденным. Не хочешь прилечь здесь, на моих удобных корнях?"
    d "Have you got lost? You look so tired, even weary, I would say. Don't you want to rest here upon my comfortable roots."

# game/script.rpy:149
translate english label3_7c44c80d:

    # "Теплые корни дерева будто бы обнимают, сладкая тяжесть болотного воздуха клонит к земле, а голова наполняется баюкающей монотонной песней дерева."
    "It appears like warm tree roots are embracing you, swamp air is pulling you to the ground and your head is filling with monotonous tree's lullaby."

# game/script.rpy:150
translate english label3_30b6af10:

    # "К сожалению, проснуться вам уже не суждено."
    "Unfortunately, it is not destined for you to awake."

# game/script.rpy:155
translate english label3_7ff9c698:

    # d "О, я помню этот лес... Я - одно из старейших деревьев в нем! Я проросло здесь еще когда не было никаких идо..."
    d "Oh I remember the forest. I'm one of the oldest trees here! I sprouted when there was no ido..."

# game/script.rpy:156
translate english label3_93d943c4:

    # "Вы услышали странный звук. Кажется, дерево захрапело."
    "You've heard an odd sound. It seems the tree is snoring."

# game/script.rpy:159
translate english label3_cfdd19e3:

    # d "Кто посмел потревожить мой... О, привет!"
    d "How dare you disturb my... Oh, hi!"

# game/script.rpy:162
translate english label3_cfdd19e3_1:

    # d "Кто посмел потревожить мой... О, привет!"
    d "How dare you disturb my... Oh, hi!"

# game/script.rpy:165
translate english label3_2fb5eae1:

    # d "Путники нынче такими невоспитанными стали!"
    d "These wayfarers are so rude nowadays."

# game/script.rpy:168
translate english label3_0e409b42:

    # d "Посмотри на меня внимательно."
    d "Look at me carefully."

# game/script.rpy:169
translate english label3_ff5939aa:

    # d "Я похоже на что-то, что умеет пользоваться дорогами? Я дерево!"
    d "Do I look like I can really use any ways? I am a tree."

# game/script.rpy:170
translate english label3_bf3e1a72:

    # d "Я не знаю никаких правильных путей. Да и тебе никуда идти не нужно, ведь тут так хорошо, так сонно, так спокойно. Лучше отдохнуть, поспать немного..."
    d "I don't know any right ways. And you don't need to go, it's so good and quiet and calm here. It's better to take a rest, to sleep for a while."

# game/script.rpy:171
translate english label3_9cb0345a:

    # e "Какой-то заколдованный круг, не правда ли?"
    e "Kind of a vicious circle, isn't it?"

# game/script.rpy:175
translate english label3_b7af9f49:

    # d "В каком смысле, почему я говорю?{w=1} Что за некультурный вопрос!"
    d "In what sense, why am I talking{w=1} What an uncultured question!"

# game/script.rpy:176
translate english label3_a67496dc:

    # d "Все деревья говорят. Но я понимаю, ты не со зла, от усталости лезут в голову всякие глупости."
    d "All trees are talking. But I understand you haven't meant anything, some stupid thoughts come up in your mind because you're tired."

# game/script.rpy:177
translate english label3_671a3399:

    # d "Тебе нужно просто прилечь и восстановить силы."
    d "You just need to lie down and relax."

# game/script.rpy:181
translate english label3_7459a137:

    # d "Ах это? Его со злобой воткнула в меня одна истеричная особа."
    d "Ah, you mean this? One hysterical lady stuck it in me maliciously."

# game/script.rpy:182
translate english label3_d24e394e:

    # d "Она истекала кровью и пошатывалась мимо, когда я просто предложило прилечь и отдохнуть на своих корнях. Вот и проявляй потом заботу!"
    d "She was bleeding and lurching while passing by and I just proposed taking some rest upon my roots. Is it worth caring after that?"

# game/script.rpy:183
translate english label3_57dfff51:

    # d "Вообще, я плохо разбираюсь в личинах людей, но кажется, она была чем-то опечалена. И что самое возмутительное, перед этим она даже не удосужилась вытереть его от своей мерзкой крови!"
    d "I am not good at recognizing mood on human faces, but it seemed she was a bit upset. But it was outrageous that she hadn't wiped her filthy blood off the blade!"

# game/script.rpy:186
translate english label3_211076d0:

    # d "А ну прекрати! Не отдам!"
    d "Stop it! I never loosen it!"

# game/script.rpy:187
translate english label3_a85292e0:

    # d "Сейчас вынешь, и я соком своим истеку."
    d "If you put it away, I'll bleed to death."

# game/script.rpy:192
translate english label3_2fb5eae1_1:

    # d "Путники нынче такими невоспитанными стали!"
    d "These wayfarers are so rude nowadays."

# game/script.rpy:202
translate english label3_2_c92db8ec:

    # "Попытка пройти по топкой трясине обернулась печальным, но предсказуемым исходом."
    "Your attempt to cross the bog turned out to have a sad but predictable outcome."

# game/script.rpy:203
translate english label3_2_755d02f1:

    # "Вас ждала долгая и мучительная смерть, несущая чувства беспомощности и неотвратимости."
    "Painful agony is awaiting, accompanied with helplessness and inevitability."

# game/script.rpy:228
translate english label4_d7ccb297:

    # r "Уйди! Я не желаю ничего слышать! Можешь так ему и сказать!"
    r "Get out! I don't want to hear anything! You can pass it straight to him."

# game/script.rpy:234
translate english label4_058abdcb:

    # r "А, ты… А кто ты, собственно такой? Никогда не видела тебя раньше."
    r "Ah, you... But who are you? I haven't seen you here anywhen."

# game/script.rpy:238
translate english label4_a1c92e09:

    # r "Ты ведь не отсюда. Ты заблудился?"
    r "You are not from here. Did you get lost?"

# game/script.rpy:244
translate english label4_2449bfe5:

    # r "Ужасно! Еще одна жертва этого проклятого места..."
    r "It's terrible! Another victim of this cursed place..."

# game/script.rpy:249
translate english label4_221d4d92:

    # r "Ты просто еще не понял, куда попал."
    r "You haven't still got where you are."

# game/script.rpy:250
translate english label4_eeb82f96:

    # r "И как плохи твои дела..."
    r "And how bad it is..."

# game/script.rpy:256
translate english label4_4d099895:

    # r "Здесь все искажено, извращенно и зацикленно."
    r "It's all so corrupted, twisted and looped in here."

# game/script.rpy:257
translate english label4_2c7cfb20:

    # r "Жизнь в этом лесу - сплошная пытка."
    r "Life in this forest is nothing but a torture."

# game/script.rpy:260
translate english label4_851e6576:

    # r "Я столько раз пыталась убежать отсюда.{w=1} Но, как видишь, даже смерть не помогла."
    r "I tried to escape so many times.{w=1} But, as you may see, even death brought me no salvation."

# game/script.rpy:266
translate english label4_6f9971dc:

    # r "Ты не видишь? Я навья. Русалка. Я убила себя в надежде на избавление. Но и это не помогло. Лес вцепился в мою душу и никогда уже не отдаст."
    r "Don't you see? I'm a ghost. A swamp undine. I committed suicide hoping for redemption. But it didn't work. The forest has seized my soul and will never let it go."

# game/script.rpy:269
translate english label4_edcf4058:

    # "Удивлен? Не бойся."
    "Are you astonished? Please, dont be afraid."

# game/script.rpy:271
translate english label4_124b783a:

    # "Стой, не бойся!"
    "No, don't be afraid!"

# game/script.rpy:275
translate english label4_c6762e68:

    # r "Я не причиню тебе вреда. Единственная в этом лесу, наверное."
    r "I won't harm you.  Maybe I'm the only creature in this forest that wouldn't."

# game/script.rpy:276
translate english label4_75215824:

    # r "Для тебя может еще есть шанс. Я дам тебе это. Навий посох. Коль на что-то направишь свет из глазниц черепа, так оно откроет тебе сокрытое."
    r "There may still be a chance for you to escape. I'll give you this. It's my magic staff. It will reveal the concealed."

# game/script.rpy:282
translate english label4_7cb6afb9:

    # r "Как давно я не слышала эти слова."
    r "I haven't heard such words for ages..."

# game/script.rpy:307
translate english label5_fb1cfd45:

    # "Похоже, вы смогли благополучно выбраться топей."
    ""

# game/script.rpy:320
translate english label5_891368d0:

    # "Почему бы не сделать еще один кружочек по болоту?"
    ""

# game/script.rpy:332
translate english label6_7f7cb42e:

    # v "Мравствуй. И откуда мы такие красивые? Какой румянец да вид свежий. Очень соблазнительно."
    v ""

# game/script.rpy:336
translate english label6_1906a94a:

    # v "А пойдем ко мне в избу, я тебя накормлю, напою да спать уложу."
    v ""

# game/script.rpy:337
translate english label6_08d00860:

    # v "Круг кружит, пруток лежит, ручеек под ним бежит, как сломаю я пруток, ворожбы настанет срок, бег воды остановится, слово крепкое смолвится."
    v ""

# game/script.rpy:349
translate english label6_8c1b9aa9:

    # v "Ах, не слушай глупую одинокую женщину, несу всякий вздор. Пойдем лучше в избу, пироги да блины остывают, я тебе угожу, раны твои исцелю да силы восстановлю."
    v ""

# game/script.rpy:357
translate english label6_47f083f5:

    # v "Мудро рассудил. Проходи через порожек аккуратно. Вот и все. Кто через порог прошел, тот моим стал. Раздевайся, тебе это все уже будет не нужно. Садись пока сюда, на лопату, а я добавлю жару в печи."
    v ""

# game/script.rpy:367
translate english label6_051f9aa5:

    # v "Путь путеюшки змеятся, лентой ситцевой кружатся, ленточкой тебя свяжу, чары в сердце положу"
    v ""

# game/script.rpy:378
translate english label6_8628a13d:

    # v "Неожиданно. А ты достойный противник."
    v ""

# game/script.rpy:383
translate english label6_4b298272:

    # v "Ну вот и славно. Пойдем, пойдем. Раздевайся, тебе это все уже будет не нужно. Садись пока сюда, на лопату, а я добавлю жару в печи."
    v ""

# game/script.rpy:389
translate english label6_43a434ca:

    # v "Я та, кого ты и искал. Коль хочешь выбраться из чащи, так я одна тебе и помогу. Но для почину тебя надо в баньке попарить да блинами накормить."
    v ""

# game/script.rpy:396
translate english label6_865731c3:

    # v "Эти мавкины причуды здесь не работают."
    v ""

# game/script.rpy:406
translate english label6_1_7fcbaef6:

    # "На миг вместо прекрасной женщины перед вами предстала уродливая мертвая старуха."
    ""

# game/script.rpy:407
translate english label6_1_2fa25595:

    # v "Ах ты лиходей срамный! Получай костяной ногой!"
    v ""

# game/script.rpy:425
translate english label7_7e9b7e70:

    # l "Кто это бродит по моему лесу? Не видал твою морду раньше. Ну, поклонись лесному хозяину!"
    l ""

# game/script.rpy:428
translate english label7_61fd44ce:

    # l "Ха-ха, ну спасибо! Давно меня никто уже хозяином сего леса не признает."
    l ""

# game/script.rpy:430
translate english label7_b1d55b75:

    # l "Вот, никто меня теперь не считает здесь хозяином."
    l ""

# game/script.rpy:434
translate english label7_fff02994:

    # l "А все из-за зла давно уж здесь поселившегося. Все думают, коль плутаем да блуждаем, коль тьма душит, так это леший повинен. Ан нет, все нечистое от того поганого капища идет!"
    l ""

# game/script.rpy:435
translate english label7_b06b7201:

    # l "Помоги нам! Разломай идола на капище! Спаси лес, а мы уж в долгу не останемся. Коль исполнишь просьбу да пожелаешь, выведу тебя самой короткой дорогой, не будь я леший."
    l ""

# game/script.rpy:439
translate english label7_8ef3f574:

    # l "Вот и славно! Скоро мы все наконец вздохнем спокойно да закатим пир на весь мир. И ты отправишься домой мед пиво пить."
    l ""

# game/script.rpy:441
translate english label7_c01f8dfc:

    # l "Эх, не зазорна трусость. Коль сам бы не трусил, вызвал бы силу нечистую на поле бранное!"
    l ""

# game/script.rpy:453
translate english label7_b9c10b34:

    # l "Помню... помню дочурку мою, девицу красну. Сгубил ее этот лес, все силы выжал. А я не смог уберечь ни лес, ни ее. Бедняжка руки на себя наложила. Жалкий я дурень..."
    l ""

# game/script.rpy:454
translate english label7_2994b664:

    # l "Вот бы можно было в живую ее обратно оборотить. Я бы все за это отдал... лишь бы в очи ее навьи более не смотреть, укора и ненависти полные. "
    l ""

# game/script.rpy:455
translate english label7_12e6bd47:

    # l "От стыда в голове будто чаща от ветра дикого шумит."
    l ""

# game/script.rpy:468
translate english label8_c1e4f4cf:

    # i "Еще один смертный. Ты пришел принести жертву?"
    i ""

# game/script.rpy:474
translate english label8_92d8360f:

    # i "Ты разговариваешь с богом этого леса! Пади ниц и принеси мне достойную жертву! Свое сердце!"
    i ""

# game/script.rpy:485
translate english label8_2d51dbc8:

    # i "Я неподвластен русалочьим чарам, глупец!"
    i ""

# game/script.rpy:486
translate english label8_6f20482c:

    # i "Я - злая, испорченная душа этого леса. Мне нужны вкусные души путников вроде тебя, забредших в этот мир случайно."
    i ""

# game/script.rpy:487
translate english label8_3674b9aa:

    # i "Если меня не будут кормить жертвами, я умру. А я очень боюсь умирать. Не будет никакого перерождения. Я не хочу. Я должен поглотить души!"
    i ""

# game/script.rpy:492
translate english label8_d544ebb6:

    # i "Ты смеешь что-то требовать от меня? Кто тебе вообще разрешил разговаривать со мной? Исчезни, ничтожество."
    i ""

# game/script.rpy:498
translate english label8_065134eb:

    # "Вы касаетесь идола и земля проваливается у вас под ногами."
    ""

# game/script.rpy:509
translate english label9_6e79d3a6:

    # t "Ну что ж… круг замкнулся. Ты снова пришел ко мне."
    t ""

# game/script.rpy:513
translate english label9_9fc46596:

    # t "Теперь ты будешь вечно ходить по этому кругу и развлекать меня, раз за разом исполняя одну и ту же пьесу. Да, реквизита здесь маловато, увы."
    t ""

# game/script.rpy:514
translate english label9_e4303249:

    # t "Но не унывай, ты скоро привыкнешь и перестанешь обращать внимание на такие мелочи..."
    t ""

# game/script.rpy:526
translate english dead_leha_4b48bfa5:

    # l "Что за палкой тут тычешь? Аль череп на ней? Не могу разглядеть, стал слаб мой взор."
    l ""

# game/script.rpy:527
translate english dead_leha_8cb69b97:

    # l "А ну пойди прочь, окаянный!"
    l ""

# game/script.rpy:541
translate english dead_acfbae39:

    # "Вы погибли!"
    ""

translate english strings:


    # game/script.rpy:119
    old "Идти вперед"
    new "Go ahead"

    # game/script.rpy:119
    old "Идти назад"
    new "Go back"

    # game/script.rpy:119
    old "Заблудиться"
    new "Go astray"

    # game/script.rpy:147
    old "Cогласиться с предложением"
    new "Accept the proposal"

    # game/script.rpy:147
    old "Cпросить о лесе"
    new "Ask about the forest"

    # game/script.rpy:147
    old "Cпросить о правильной дороге"
    new "Ask about the right way"

    # game/script.rpy:147
    old "Cпросить, почему дерево говорит"
    new "Ask why the tree is talking"

    # game/script.rpy:147
    old "Cпросить про торчащий из коры кинжал"
    new "Ask about the dagger in tree's bark"

    # game/script.rpy:147
    old "Проигнорировать дерево"
    new "Ignore the tree"

    # game/script.rpy:157
    old "Обратить на себя внимание вежливым кашлем"
    new "Drive the attention with a polite cough"

    # game/script.rpy:157
    old "Пнуть дерево"
    new "Punch the tree"

    # game/script.rpy:157
    old "Уйти"
    new "Go away"

    # game/script.rpy:184
    old "Попытаться вытащить кинжал!"
    new "Try to put the dagger away"

    # game/script.rpy:200
    old "Идти дальше через болото"
    new "Go further through the swamp"

    # game/script.rpy:200
    old "Вернуться назад"
    new "Come back"

    # game/script.rpy:229
    old "Поинтересоваться, что происходит"
    new "Wonder what's going on"

    # game/script.rpy:239
    old "Согласиться"
    new "Agree"

    # game/script.rpy:239
    old "Отрицать"
    new "Deny"

    # game/script.rpy:261
    old "Переспросить про смерть"
    new "Ask to specify"

    # game/script.rpy:267
    old "Промолчать"
    new "Keep silence"

    # game/script.rpy:267
    old "Попятиться"
    new "Back away"

    # game/script.rpy:277
    old "Поблагодарить"
    new "Thank her"

    # game/script.rpy:283
    old "Получен Навий посох."
    new "An undine's staff obtained"

    # game/script.rpy:312
    old "Идти налево"
    new ""

    # game/script.rpy:312
    old "Идти направо"
    new ""

    # game/script.rpy:343
    old "Сопротивляться чарам"
    new ""

    # game/script.rpy:355
    old "Принять предложение"
    new ""

    # game/script.rpy:355
    old "Бежать куда глаза глядят"
    new ""

    # game/script.rpy:355
    old "Спросить про правильный путь из леса"
    new ""

    # game/script.rpy:355
    old "Спросить, кто она"
    new ""

    # game/script.rpy:355
    old "Применить навий посох"
    new ""

    # game/script.rpy:372
    old "Подчиниться чарам"
    new ""

    # game/script.rpy:412
    old "Уклониться"
    new ""

    # game/script.rpy:426
    old "Поклониться"
    new ""

    # game/script.rpy:426
    old "Отказаться"
    new ""

    # game/script.rpy:437
    old "Использовать посох навьи"
    new ""

    # game/script.rpy:447
    old "Зачаровать"
    new ""

    # game/script.rpy:456
    old "Идти дальше"
    new ""

    # game/script.rpy:472
    old "Спросить про жертву"
    new ""

    # game/script.rpy:472
    old "Применить посох"
    new ""

    # game/script.rpy:472
    old "Потребовать освободить себя"
    new ""

    # game/script.rpy:472
    old "Разломать идол"
    new ""

    # game/script.rpy:482
    old "Начертить посохом руну"
    new ""

    # game/script.rpy:528
    old "Убежать"
    new ""

